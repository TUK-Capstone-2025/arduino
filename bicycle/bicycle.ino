...
// 블루투스 통신 핀 설정
#define BT_RX 7              // 블루투스 모듈 RX 핀
#define BT_TX 8              // 블루투스 모듈 TX 핀
...
const float TILT_ANGLE_THRESHOLD = 45.0;// 넘어짐 기울기 각도 임계값 (각도)
const unsigned long RECOVERY_TIME = 10000; // 정상 범주 복귀 허용 시간 (10초)
const int LIGHT_THRESHOLD = 300;        // 조도 센서 임계값
...
void setup() {
  ...
  // 블루투스 초기화 메시지
  Serial.println("블루투스 모듈 초기화 완료. 슬레이브 연결 시도 중...");
}
...
void checkLightSensor() {
  ...
  if (lightValue > LIGHT_THRESHOLD) {
    ...
    Serial.println("어두움 (전방 라이트 점등)");
  } else {
    digitalWrite(LED_PIN, LOW);
    bluetooth.println("OFF");
    Serial.println("밝음 (전방 라이트 소등)");
  }
}